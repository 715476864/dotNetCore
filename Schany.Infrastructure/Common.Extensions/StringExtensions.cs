...
namespace Schany.Infrastructure.Common.Extensions
{
    ...
    public static class StringExtensions
    {
        ...
        /// <returns></returns>
        public static bool GetSexByCardId(this string cardId)
        {
            string sexChar = "1";
            if (cardId.Length == 15)
            {
                //15位身份证最后一位数字奇数为男性，偶数为女性。
                sexChar = cardId.Substring(14, 1);
            }
            else if (cardId.Length == 18)
            {
                //18位身份证倒数第二位数字奇数为男性，偶数为女性。
                sexChar = cardId.Substring(16, 1);
            }

            if (Convert.ToInt16(sexChar) % 2 == 0)
            {
                return false;
            }
            else
        ...
        public static DateTime GetBirthDayByCardId(this string cardId)
        {
            if (!cardId.IsIdentityCard())
                throw new Exception("获取出生日期发生错误：身份证号码验证失败");
            string birthday = "";
